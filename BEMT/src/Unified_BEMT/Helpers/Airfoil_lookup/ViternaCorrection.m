function [Cl,Cd] = ViternaCorrection(aspect_ratio, Cl_orig, Cd_orig, AoA, alpha_s, Cds, Clmax)
%{
...
Cl_nan = zeros(size(Cl_orig));
Cd_nan = zeros(size(Cl_orig));


Cl_nan(isnan(Cl_orig))=1;
Cd_nan(isnan(Cd_orig))=1;

Cl_orig(isnan(Cl_orig)) =0;
Cd_orig(isnan(Cd_orig)) =0;
...
Cl_viterna = Cdmax*sin(2*deg2rad(AoA))/2+K_l*(cos(deg2rad(AoA)).^2)./sin(deg2rad(AoA));
Cd_viterna = Cdmax*(sin(deg2rad(AoA))).^2+K_d*cos(deg2rad(AoA));

Cl = Cl_viterna.*Cl_nan + Cl_orig;
Cd = Cd_viterna.*Cd_nan + Cd_orig;
