function lambda = getLambda_Leish(rotor,lambda_ext,lambda_T,F,r,psi,cl_alpha)
%{
...
b = rotor.solidity*cl_alpha.*(mu+r.^2)./(16*F.*r.*(r+lambda_T.*sin(psi)))-lambda_ext/2;

lambda = sqrt(b.^2+rotor.solidity*cl_alpha.*rotor.pitch.*(mu+r.^2)./(8*F.*r))-b;
