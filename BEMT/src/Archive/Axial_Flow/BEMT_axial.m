...
if strcmpi(rotorsystem.type,"single")
    ...
    geometric_pitch = getPitch(r,rotorsystem.rotor(1).twist_type,rotorsystem.rotor(1).twistdeg,rotorsystem.rotor(1).pitch_root);

    pitchdeg = geometric_pitch+ones(size(geometric_pitch))*rotorsystem.state.collective;
    
    
    
    single.rotor(1).pitch = deg2rad(pitchdeg); %rad
    ...
    pitchdeg = coaxial.state.collective;
